...
<div class="container">
  ...
  <div>
    <label>Guess</label>
    <div>
      <input id="guess" type="text" placeholder="What's the answer?">
    </div>
  </div>

  <div>
    <div>
      <button id="answerBtn" class="answerBtn">Check</button>
    </div>
  </div>
...
for (let i = 0; i < guesses.length; i++) { if (answers[i].value===guesses[i].value) { guesses[i].validation=true; } else
  ...
  answerBtn.innerText='Correct!' ; answerBtn.className='button is-success' ;
  answerContainer.className='card has-background-success has-text-dark' ; game.correctGuesses +=1;
  streak.innerText=`${game.correctGuesses}`; setTimeout(()=> {
  reset();
